...
pub fn encode(operator_index_list: &[usize], operator_count: usize) -> EncodeResult {
    let mut operator_index_freq_table = vec![0usize; operator_count];
    ...
    let operator_index_length_table = create_operator_index_length_table(root, operator_count);
    let canonical = create_canonical_tables(&operator_index_length_table);

    let encoded: Vec<(u32, usize)> = operator_index_list
        .iter()
        .map(|&operator_index| {
            (
                canonical.operator_index_code_table[operator_index],
                operator_index_length_table[operator_index],
            )
        })
        .collect();
...
pub fn decode(
    ...
    while decoded.len() < operator_index_count {
        let Some(bit) = reader.read_msb(1) else { break };
        value = (value << 1) + bit;
        length += 1;
            ...
            && value >= canonical.length_first_operator_index_code_table[length]
        {
            ...
            if offset < canonical.length_count_table[length] {
                ...
                let operator_index = canonical.sorted_index_list[base + offset];
